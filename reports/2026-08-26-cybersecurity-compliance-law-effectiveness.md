...
1. **"Compliance Is a Floor, Not a Shield" (mechanism + evidence table)** — category: fact-sheet. Euclid's structural argument (compliance mandates operate defender-side and can only affect threat categories where target selection depends on the defender's baseline posture) plus the category-by-category evidence table Tufte built are together a durable, reusable lens for any future "did this policy/regulation actually work" question — the same kind of standing analytical tool as the Evidence Tier Framework or Campaign vs. Actor Attribution fact-sheets already in the Library. Status: **Submitted — [PR #22](https://github.com/raceBannon99/nexus-artifacts/pull/22), awaiting merge.** This mechanism proved independently reusable in a second domain (AI-lab RSI licensing deterrence, `reports/2026-09-21-rsi-moratorium-legislation-draft.md`), so the submitted fact-sheet, `fact-sheets/compliance-floor-not-shield-deterrence-diagnostic.md`, covers both applications rather than duplicating this one alone — see that report's own Library Recommendations section.
...
**2026-09-21:** Candidate 1 ("Compliance Is a Floor, Not a Shield") approved by Rick for submission — merged with a second, independent application of the same mechanism from `reports/2026-09-21-rsi-moratorium-legislation-draft.md` into one fact-sheet, submitted as [PR #22](https://github.com/raceBannon99/nexus-artifacts/pull/22) against `nexus-artifacts`. Candidate 2 (book cross-reference note) was not part of this request and remains recommended, awaiting decision. Structural/presentational only — no new claims, no other agent re-run.
