...
```mermaid
flowchart LR
    A["Free content\nBook reviews, website,\nnewsletter, social"] -->|builds| B["Reach + Credibility\nThe Vanguard\n≈385K global (central est.)"]
    B -->|monetized as| C["Speaker's Bureau fees\nSponsorships (Books in Cyber)\nDonor gifts (Resilience, Hedy)"]
    C -->|funds| A

    B -.->|independence condition| D{{"Does the funding\nmechanism protect\nor erode B?"}}
    D -.->|protects| B
    D -.->|erodes| E["Credibility decay\n(Gartner/MQ pattern —\nlatent, not observed here)"]

    classDef core fill:#1c1c1e,stroke:#38383a,color:#f5f5f7;
    classDef risk fill:#3a1f1f,stroke:#7a3b3b,color:#f5f5f7;
    class A,B,C core;
    class D,E risk;
```
...
**What happens to this row if Rick approves it** (per [[Nexus Artifact Repository]]'s "Closing the Loop" section): status updates to "Submitted — PR #N, awaiting merge" the moment a Pull Request opens against `nexus-artifacts` — not left saying "awaiting decision" once a decision has actually been made. Once that PR merges, a further update changes status to "Added to Library," linked to the artifact's final path, **and** a new entry is added to the Sources section above citing it directly. This report doesn't go stale the moment the recommendation is acted on.
